...
use station_shared::{
    ...
    db::{
        decode_optional_string, decode_required_string, prefix_scope_arg, prefix_scope_sql,
        SqlDialect,
    },
};
use zip::ZipArchive;
...
async fn fetch_identity_names(
    ...
    let station_uid_prefix_match =
        prefix_scope_sql(dialect, "station_uid", STATION_UID_PREFIX.len());
    ...
    .bind(prefix_scope_arg(dialect, STATION_UID_PREFIX))
    .fetch_all(&mut **tx)
    .await?;
...
mod tests {
    ...
    fn mysql_station_uid_prefix_match_uses_binary_like_with_escaped_prefix() {
        assert_eq!(
            prefix_scope_sql(SqlDialect::Mysql, "station_uid", STATION_UID_PREFIX.len()),
            "station_uid COLLATE utf8mb4_bin LIKE ? ESCAPE '\\\\'"
        );
        assert_eq!(
            prefix_scope_arg(SqlDialect::Mysql, STATION_UID_PREFIX),
            "stn\\_n02\\_%"
        );
    }
