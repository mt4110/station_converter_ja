...
DB に接続できない場合は `503` で同 shape を返し、`dataset.status` は `unknown` です。
...
latest available MLIT N02 snapshot のうち、この DB が ingest 済みの metadata を含む状態を返します。
...
共通 error envelope は `error.code` / `error.message` を必ず持ちます。
machine-readable context がある場合だけ optional `error.detail` を返します。
`detail.kind` は payload category、`detail.issues[]` は field / parameter 単位の補助情報です。
既存 consumer は `code` / `message` だけを読み続けられます。
...
public `station-api` contract を変える場合は、同じ patch で次を揃えます。

- `worker/api` の route annotation / DTO / error shape
- generated `/openapi.json`
- `API_SPEC.md` の example と説明
- `frontend/src/lib/generated/station-openapi.ts`
- `frontend/src/lib/station-sdk.ts`
